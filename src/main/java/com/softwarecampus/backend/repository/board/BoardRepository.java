...
public interface BoardRepository extends JpaRepository<Board, Long> {
        ...
        @Query(value = "SELECT new com.softwarecampus.backend.dto.board.BoardListResponseDTO(b.id,MAX(b.category),MAX(b.title),MAX(b.secret),MAX(a.userName),MAX(a.id),sum(case when c.isDeleted=false then 1 else 0 end),MAX(b.hits),count(distinct r.id),max(b.createdAt)) from Board b left join b.boardRecommends r left join b.comments c join b.account a "
                        +
        ...
        @Query(value = "SELECT new com.softwarecampus.backend.dto.board.BoardListResponseDTO(b.id,MAX(b.category),MAX(b.title),MAX(b.secret),MAX(a.userName),MAX(a.id),sum(case when c.isDeleted=false then 1 else 0 end),MAX(b.hits),count(distinct r.id),max(b.createdAt)) from Board b left join b.boardRecommends r left join b.comments c join b.account a "
                        +
                        "WHERE b.category=:category and b.title like :searchText and b.isDeleted=false GROUP BY b.id", countQuery = "select count(b) from Board b WHERE b.category=:category and b.title like :searchText and b.isDeleted=false")
        Page<BoardListResponseDTO> findBoardsByTitle(@Param("category") BoardCategory category,
                        @Param("searchText") String searchText, Pageable pageable);

        @Query(value = "SELECT new com.softwarecampus.backend.dto.board.BoardListResponseDTO(b.id,MAX(b.category),MAX(b.title),MAX(b.secret),MAX(a.userName),MAX(a.id),sum(case when c.isDeleted=false then 1 else 0 end),MAX(b.hits),count(distinct r.id),max(b.createdAt)) from Board b left join b.boardRecommends r left join b.comments c join b.account a "
                        +
                        "WHERE b.category=:category and b.text like :searchText and b.isDeleted=false GROUP BY b.id", countQuery = "select count(b) from Board b WHERE b.category=:category and b.text like :searchText and b.isDeleted=false")
