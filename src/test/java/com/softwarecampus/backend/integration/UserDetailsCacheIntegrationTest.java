...
class UserDetailsCacheIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        // 기존 데이터 정리 (FK 체크 비활성화)
        jdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 0");
        accountRepository.deleteAll();
        jdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 1");

        // 캐시 초기화 및 Redis 연결 확인
        if (cacheManager != null) {
            try {
                // 간단한 Redis 작업 시도 (실제 연결 확인)
                Cache testCache = cacheManager.getCache("userDetails");
                if (testCache != null) {
                    testCache.get("connection_test");
                }
                ...
                System.out.println("⚠️ Redis 연결 실패로 캐시 초기화 스킵: " + e.getMessage());
    ...
    @AfterEach
    void tearDown() {
        // 테스트 후 데이터 정리 (FK 체크 비활성화)
        jdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 0");
        accountRepository.deleteAll();
        jdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 1");

        // 캐시 정리
        if (isRedisAvailable && cacheManager != null) {
            try {
                cacheManager.getCacheNames().forEach(cacheName -> {
                    Cache cache = cacheManager.getCache(cacheName);
                    if (cache != null) {
                ...
                System.out.println("⚠️ Redis 정리 실패: " + e.getMessage());
            }
        }
    }
    ...
    void worksWithoutRedis() {
        // Redis가 설정되어 있는데 연결이 안되면 @Cacheable에서 에러가 날 수 있음
        if (cacheManager != null && !isRedisAvailable) {
            System.out.println("⚠️ Redis 설정됨 그러나 연결 불가 - 테스트 스킵");
            return;
        }
