...
### 3. 브랜치 전략 (AI Code Review)
- **별도 브랜치 사용**: 중요한 리팩터링, 기능 추가, 또는 AI 코드 리뷰가 필요한 작업은 반드시 `main`이나 `develop` 브랜치에 직접 커밋하지 말고, 별도의 기능 브랜치(예: `feat/xxx`, `refactor/xxx`)를 생성하여 작업합니다.
