# SMTP 이메일 인증 기능 - 전략 설계

## 📋 개요

**목적:** 회원가입 및 비밀번호 재설정 시 이메일 인증 코드 방식 구현  
**방식:** 6자리 숫자 코드 전송 및 검증  
**SMTP:** Google Gmail 계정 사용  
**저장소:** DB (email_verification 테이블)

---

## 🎯 구현 범위

### 1. 인증 시나리오

#### A. 회원가입 이메일 인증
```
1. 사용자가 이메일 입력
2. POST /api/auth/email/send-verification
   → 6자리 코드 생성 + 이메일 발송
3. 사용자가 코드 입력
4. POST /api/auth/email/verify
   → 코드 검증 성공
5. POST /api/auth/signup
   → 이메일 인증 완료 상태에서만 가입 허용
```

#### B. 비밀번호 재설정 이메일 인증
```
1. 사용자가 이메일 입력 (가입된 계정)
2. POST /api/auth/email/send-reset-code
   → 6자리 코드 생성 + 이메일 발송
3. 사용자가 코드 입력
4. POST /api/auth/email/verify-reset
   → 코드 검증 + 임시 토큰 발급
5. POST /api/auth/password/reset
   → 임시 토큰으로 비밀번호 재설정
```

---

## 🏗️ 아키텍처 설계

### 1. 기술 스택 추가

#### 의존성 (pom.xml)
```xml
<!-- Spring Boot Mail Starter -->
<dependency>
    <groupId>org.springframework.boot</groupId>
    <artifactId>spring-boot-starter-mail</artifactId>
</dependency>
```

#### SMTP 설정 (application.properties)
```properties
# Gmail SMTP 설정
spring.mail.host=smtp.gmail.com
spring.mail.port=587
spring.mail.username=${MAIL_USERNAME}
spring.mail.password=${MAIL_APP_PASSWORD}
spring.mail.properties.mail.smtp.auth=true
spring.mail.properties.mail.smtp.starttls.enable=true
spring.mail.properties.mail.smtp.starttls.required=true
spring.mail.properties.mail.smtp.connectiontimeout=5000
spring.mail.properties.mail.smtp.timeout=5000
spring.mail.properties.mail.smtp.writetimeout=5000

# 이메일 발신자 정보
spring.mail.properties.mail.smtp.from=noreply@softwarecampus.com

# 이메일 인증 정책
email.verification.code-length=6
email.verification.expiry-minutes=3
email.verification.max-attempts=5
email.verification.resend-cooldown-seconds=60
email.verification.block-duration-minutes=30
```

#### 환경변수 (.env)
```properties
MAIL_USERNAME=your-gmail@gmail.com
MAIL_APP_PASSWORD=your-app-specific-password
```

**⚠️ 보안 경고:**
- **실제 비밀번호를 문서나 Git에 절대 포함하지 마세요**
- `.env` 파일은 반드시 `.gitignore`에 추가하세요
- 비밀번호 노출 시 즉시 폐기 후 재생성 필요

---

### 2. 데이터 모델

#### EmailVerification 엔티티 (DB 저장용)
```java
@Entity
@Table(name = "email_verification")
@EntityListeners(AuditingEntityListener.class)
public class EmailVerification {
    
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;
    
    @Column(nullable = false, length = 100)
    private String email;
    
    @Column(nullable = false, length = 6)
    private String code;  // 6자리 숫자 코드
    
    @Enumerated(EnumType.STRING)
    @Column(nullable = false, length = 20)
    private VerificationType type;
    
    @Column(nullable = false)
    private LocalDateTime expiresAt;
    
    @Column(nullable = false)
    private boolean verified = false;
    
    @Column(nullable = false)
    private int attempts = 0;
    
    @CreatedDate
    @Column(nullable = false, updatable = false)
    private LocalDateTime createdAt;
    
    private LocalDateTime verifiedAt;
    
    private LocalDateTime blockedUntil;  // 재시도 차단 종료 시간
    
    // 인증 완료 처리
    public void markAsVerified() {
        this.verified = true;
        this.verifiedAt = LocalDateTime.now();
    }
    
    // 시도 횟수 증가
    public void incrementAttempts() {
        this.attempts++;
    }
    
    // 만료 여부 확인
    public boolean isExpired() {
        return LocalDateTime.now().isAfter(expiresAt);
    }
    
    // 최대 시도 초과 여부
    public boolean isAttemptsExceeded() {
        return attempts >= 5;
    }
    
    // 차단 여부 확인
    public boolean isBlocked() {
        return blockedUntil != null && LocalDateTime.now().isBefore(blockedUntil);
    }
    
    // 차단 설정 (30분)
    public void blockForDuration(int minutes) {
        this.blockedUntil = LocalDateTime.now().plusMinutes(minutes);
    }
    
    // 남은 차단 시간 (분)
    public long getRemainingBlockMinutes() {
        if (blockedUntil == null || LocalDateTime.now().isAfter(blockedUntil)) {
            return 0;
        }
        return ChronoUnit.MINUTES.between(LocalDateTime.now(), blockedUntil);
    }
}
```

#### VerificationType Enum
```java
public enum VerificationType {
    SIGNUP("회원가입"),
    PASSWORD_RESET("비밀번호 재설정");
    
    private final String description;
    
    VerificationType(String description) {
        this.description = description;
    }
    
    public String getDescription() {
        return description;
    }
}
```

---

### 3. 레이어 구조

#### 패키지 구조
```
src/main/java/com/softwarecampus/backend/
├── domain/
│   ├── EmailVerification.java
│   └── common/
│       └── VerificationType.java
├── repository/
│   └── user/
│       └── EmailVerificationRepository.java
├── service/
│   └── user/
│       └── email/
│           ├── EmailVerificationService.java
│           ├── EmailVerificationServiceImpl.java
│           ├── EmailSendService.java
│           └── EmailSendServiceImpl.java
├── controller/
│   └── user/
│       └── EmailVerificationController.java
├── dto/
│   └── user/
│       ├── SendVerificationRequest.java
│       ├── SendVerificationResponse.java
│       ├── VerifyCodeRequest.java
│       └── VerifyCodeResponse.java
├── exception/
│   ├── EmailVerificationException.java
│   ├── EmailVerificationBlockedException.java
│   ├── EmailSendException.java
│   └── VerificationCodeExpiredException.java
└── util/
    └── VerificationCodeGenerator.java
```

---

## 📝 API 명세

### 1. 인증 코드 발송 (회원가입)

**Endpoint:** `POST /api/auth/email/send-verification`

**Request:**
```json
{
  "email": "user@example.com",
  "type": "SIGNUP"
}
```

**Response (200 OK):**
```json
{
  "message": "인증 코드가 이메일로 전송되었습니다",
  "email": "user@example.com",
  "expiresIn": 180,
  "canResendAt": "2025-01-15T10:31:00Z"
}
```

**Error Cases:**
- 400: 이미 가입된 이메일
- 400: 이메일 형식 오류
- 429: 재전송 쿨다운 (60초 이내)
- 429: 최대 시도 초과로 인한 차단 (30분)
- 500: 이메일 발송 실패

---

### 2. 인증 코드 검증 (회원가입)

**Endpoint:** `POST /api/auth/email/verify`

**Request:**
```json
{
  "email": "user@example.com",
  "code": "123456",
  "type": "SIGNUP"
}
```

**Response (200 OK):**
```json
{
  "verified": true,
  "message": "이메일 인증이 완료되었습니다"
}
```

**Error Cases:**
- 400: 인증 코드 불일치
- 400: 인증 코드 만료
- 429: 최대 시도 횟수 초과 (5회) - 30분 차단
- 404: 인증 요청 없음

---

### 3. 비밀번호 재설정 코드 발송

**Endpoint:** `POST /api/auth/email/send-reset-code`

**Request:**
```json
{
  "email": "user@example.com"
}
```

**Response (200 OK):**
```json
{
  "message": "비밀번호 재설정 코드가 이메일로 전송되었습니다",
  "email": "user@example.com",
  "expiresIn": 180,
  "canResendAt": "2025-01-15T10:31:00Z"
}
```

**Error Cases:**
- 404: 가입되지 않은 이메일
- 429: 재전송 쿨다운 (60초 이내)
- 429: 최대 시도 초과로 인한 차단 (30분)
- 500: 이메일 발송 실패

---

### 4. 비밀번호 재설정 코드 검증

**Endpoint:** `POST /api/auth/email/verify-reset`

**Request:**
```json
{
  "email": "user@example.com",
  "code": "123456"
}
```

**Response (200 OK):**
```json
{
  "verified": true,
  "resetToken": "example.jwt.token",
  "message": "인증이 완료되었습니다. 비밀번호를 재설정하세요",
  "expiresIn": 300
}
```

**⚠️ 참고:** `resetToken`은 예시 값입니다. 실제 JWT는 서버에서 생성됩니다.

**Error Cases:**
- 400: 인증 코드 불일치
- 400: 인증 코드 만료
- 429: 최대 시도 횟수 초과 (5회) - 30분 차단
- 404: 인증 요청 없음

---

## 🔐 보안 정책

### 1. 인증 코드 생성
- **형식:** 6자리 숫자 (000000 ~ 999999)
- **생성:** `SecureRandom` 사용 (예측 불가능)
- **중복 방지:** 동일 이메일/타입에 대해 기존 코드 무효화 후 신규 생성

### 2. 유효 기간
- **만료 시간:** 3분 (180초)
- **만료 체크:** 검증 시마다 `expiresAt` 확인

### 3. 시도 제한
- **최대 시도:** 5회
- **초과 시:** 해당 이메일 30분 차단, 코드 무효화
- **차단 기간:** 30분 (재발송 및 검증 모두 차단)
- **카운터 초기화:** 차단 해제 후 새 코드 발송 시

### 4. 재전송 제한
- **쿨다운:** 60초
- **체크 방식:** DB의 `createdAt` 기준
- **목적:** 이메일 스패 방지

### 5. 코드 재사용 방지
- **인증 완료 시:** `verified = true` 설정
- **검증 로직:** `verified == false` 인 코드만 허용

### 6. 데이터 정리
- **자동 삭제:** 만료 24시간 후 배치 작업으로 삭제

---

## 📧 이메일 템플릿

### HTML 템플릿 (회원가입)

**제목:** `[소프트웨어캠퍼스] 이메일 인증 코드`

**본문:**
```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
        .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        .header { background-color: #4A90E2; color: white; padding: 20px; text-align: center; }
        .content { padding: 30px 20px; background-color: #f9f9f9; }
        .code-box { background-color: #fff; border: 2px solid #4A90E2; border-radius: 8px; 
                    padding: 20px; text-align: center; margin: 20px 0; }
        .code { font-size: 32px; font-weight: bold; color: #4A90E2; letter-spacing: 5px; }
        .footer { text-align: center; padding: 20px; font-size: 12px; color: #999; }
        .warning { color: #E74C3C; font-size: 14px; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>소프트웨어캠퍼스</h1>
        </div>
        <div class="content">
            <h2>이메일 인증 코드</h2>
            <p>안녕하세요,</p>
            <p>회원가입을 완료하기 위해 아래 인증 코드를 입력해주세요.</p>
            
            <div class="code-box">
                <div class="code">${CODE}</div>
            </div>
            
            <p class="warning">⚠️ 이 코드는 <strong>3분</strong> 동안 유효합니다.</p>
            <p class="warning">⚠️ 인증 코드를 타인과 공유하지 마세요.</p>
            
            <p>본인이 요청하지 않은 경우, 이 이메일을 무시하셔도 됩니다.</p>
        </div>
        <div class="footer">
            <p>© 2025 소프트웨어캠퍼스. All rights reserved.</p>
            <p>본 메일은 발신 전용입니다.</p>
        </div>
    </div>
</body>
</html>
```

### HTML 템플릿 (비밀번호 재설정)

**제목:** `[소프트웨어캠퍼스] 비밀번호 재설정 코드`

**본문:** (회원가입 템플릿과 유사, 제목 및 안내 문구만 변경)

---

## 🛠️ 구현 상세

### 1. VerificationCodeGenerator (유틸)

```java
public class VerificationCodeGenerator {
    
    private static final SecureRandom RANDOM = new SecureRandom();
    private static final int CODE_LENGTH = 6;
    
    private VerificationCodeGenerator() {
        throw new UnsupportedOperationException("Utility class");
    }
    
    /**
     * 6자리 숫자 인증 코드 생성 (암호학적으로 안전한 난수)
     * SecureRandom의 바이트 배열 기반으로 더 강력한 무작위성 보장
     * @return 000000 ~ 999999
     */
    public static String generate() {
        // 바이트 배열 기반으로 난수 생성 (더 강력한 무작위성)
        byte[] randomBytes = new byte[4];
        RANDOM.nextBytes(randomBytes);
        
        // 바이트를 정수로 변환 (절댓값 처리)
        int randomInt = Math.abs(ByteBuffer.wrap(randomBytes).getInt());
        
        // 6자리 범위로 제한 (0 ~ 999999)
        int code = randomInt % 1000000;
        
        return String.format("%06d", code);
    }
}
```

---

### 2. EmailVerificationRepository

```java
public interface EmailVerificationRepository extends JpaRepository<EmailVerification, Long> {
    
    Optional<EmailVerification> findByEmailAndTypeAndVerifiedFalse(
        String email, 
        VerificationType type
    );
    
    Optional<EmailVerification> findTopByEmailAndTypeOrderByCreatedAtDesc(
        String email,
        VerificationType type
    );
    
    void deleteByExpiresAtBeforeAndVerifiedTrue(LocalDateTime threshold);
    
    boolean existsByEmailAndTypeAndVerifiedFalseAndCreatedAtAfter(
        String email,
        VerificationType type,
        LocalDateTime cooldownTime
    );
    
    Optional<EmailVerification> findByEmailAndTypeAndBlockedUntilAfter(
        String email,
        VerificationType type,
        LocalDateTime now
    );
}
```

---

### 3. EmailSendService (이메일 발송)

```java
public interface EmailSendService {
    void sendVerificationCode(String to, String code, VerificationType type);
}

@Service
@RequiredArgsConstructor
public class EmailSendServiceImpl implements EmailSendService {
    
    private final JavaMailSender mailSender;
    
    @Value("${spring.mail.properties.mail.smtp.from}")
    private String fromEmail;
    
    @Override
    public void sendVerificationCode(String to, String code, VerificationType type) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
            
            helper.setFrom(fromEmail);
            helper.setTo(to);
            helper.setSubject(getSubject(type));
            helper.setText(getHtmlContent(code, type), true);
            
            mailSender.send(message);
            
        } catch (MessagingException e) {
            throw new EmailSendException("이메일 발송에 실패했습니다", e);
        }
    }
    
    private String getSubject(VerificationType type) {
        return type == VerificationType.SIGNUP
            ? "[소프트웨어캠퍼스] 이메일 인증 코드"
            : "[소프트웨어캠퍼스] 비밀번호 재설정 코드";
    }
    
    private String getHtmlContent(String code, VerificationType type) {
        // HTML 템플릿 반환 (위의 템플릿 사용)
        // 실제 구현 시 Thymeleaf 또는 String 치환 사용
    }
}
```

---

### 4. EmailVerificationService (비즈니스 로직)

```java
public interface EmailVerificationService {
    
    SendVerificationResponse sendVerificationCode(SendVerificationRequest request);
    
    VerifyCodeResponse verifyCode(VerifyCodeRequest request);
    
    SendVerificationResponse sendPasswordResetCode(String email);
    
    VerifyCodeResponse verifyPasswordResetCode(String email, String code);
}

@Service
@Transactional
@RequiredArgsConstructor
public class EmailVerificationServiceImpl implements EmailVerificationService {
    
    private final EmailVerificationRepository verificationRepository;
    private final AccountRepository accountRepository;
    private final EmailSendService emailSendService;
    
    @Value("${email.verification.expiry-minutes}")
    private int expiryMinutes;
    
    @Value("${email.verification.resend-cooldown-seconds}")
    private int resendCooldownSeconds;
    
    @Value("${email.verification.max-attempts}")
    private int maxAttempts;
    
    @Value("${email.verification.block-duration-minutes}")
    private int blockDurationMinutes;
    
    @Override
    public SendVerificationResponse sendVerificationCode(SendVerificationRequest request) {
        // 1. 이메일 형식 검증
        if (!EmailUtils.isValidFormat(request.getEmail())) {
            throw new InvalidEmailFormatException("유효하지 않은 이메일 형식입니다");
        }
        
        // 2. 차단 여부 확인 (최대 시도 초과)
        checkBlockStatus(request.getEmail(), request.getType());
        
        // 3. 회원가입 인증의 경우 중복 이메일 체크
        if (request.getType() == VerificationType.SIGNUP 
            && accountRepository.existsByEmail(request.getEmail())) {
            throw new DuplicateEmailException("이미 가입된 이메일입니다");
        }
        
        // 4. 재전송 쿨다운 체크
        checkResendCooldown(request.getEmail(), request.getType());
        
        // 4. 기존 인증 코드 무효화
        invalidateExistingVerification(request.getEmail(), request.getType());
        
        // 5. 새 인증 코드 생성
        String code = VerificationCodeGenerator.generate();
        LocalDateTime expiresAt = LocalDateTime.now().plusMinutes(expiryMinutes);
        
        EmailVerification verification = EmailVerification.builder()
            .email(request.getEmail())
            .code(code)
            .type(request.getType())
            .expiresAt(expiresAt)
            .build();
        
        verificationRepository.save(verification);
        
        // 6. 이메일 발송
        emailSendService.sendVerificationCode(request.getEmail(), code, request.getType());
        
        // 7. 응답 생성
        return SendVerificationResponse.builder()
            .message("인증 코드가 이메일로 전송되었습니다")
            .email(request.getEmail())
            .expiresIn(expiryMinutes * 60)
            .canResendAt(LocalDateTime.now().plusSeconds(resendCooldownSeconds))
            .build();
    }
    
    @Override
    public VerifyCodeResponse verifyCode(VerifyCodeRequest request) {
        // 1. 인증 코드 조회
        EmailVerification verification = verificationRepository
            .findByEmailAndTypeAndVerifiedFalse(request.getEmail(), request.getType())
            .orElseThrow(() -> new VerificationNotFoundException("인증 요청을 찾을 수 없습니다"));
        
        // 2. 만료 확인
        if (verification.isExpired()) {
            throw new VerificationCodeExpiredException("인증 코드가 만료되었습니다");
        }
        
        // 3. 최대 시도 횟수 확인
        if (verification.isAttemptsExceeded()) {
            // 30분 차단 설정
            verification.blockForDuration(blockDurationMinutes);
            verificationRepository.save(verification);
            
            throw new MaxAttemptsExceededException(
                String.format("최대 시도 횟수를 초과했습니다. %d분 후에 다시 시도하세요", blockDurationMinutes)
            );
        }
        
        // 4. 코드 검증
        if (!verification.getCode().equals(request.getCode())) {
            verification.incrementAttempts();
            verificationRepository.save(verification);
            throw new InvalidVerificationCodeException("인증 코드가 일치하지 않습니다");
        }
        
        // 5. 인증 완료 처리
        verification.markAsVerified();
        verificationRepository.save(verification);
        
        return VerifyCodeResponse.builder()
            .verified(true)
            .message("이메일 인증이 완료되었습니다")
            .build();
    }
    
    private void checkBlockStatus(String email, VerificationType type) {
        Optional<EmailVerification> blocked = verificationRepository
            .findTopByEmailAndTypeOrderByCreatedAtDesc(email, type);
        
        if (blocked.isPresent() && blocked.get().isBlocked()) {
            long minutesLeft = blocked.get().getRemainingBlockMinutes();
            throw new EmailVerificationBlockedException(
                String.format("최대 시도 횟수 초과로 차단되었습니다. %d분 후에 다시 시도하세요", minutesLeft)
            );
        }
    }
    
    private void checkResendCooldown(String email, VerificationType type) {
        LocalDateTime cooldownTime = LocalDateTime.now().minusSeconds(resendCooldownSeconds);
        
        if (verificationRepository.existsByEmailAndTypeAndVerifiedFalseAndCreatedAtAfter(
                email, type, cooldownTime)) {
            throw new ResendCooldownException(
                String.format("%d초 후에 재전송할 수 있습니다", resendCooldownSeconds));
        }
    }
    
    private void invalidateExistingVerification(String email, VerificationType type) {
        verificationRepository.findByEmailAndTypeAndVerifiedFalse(email, type)
            .ifPresent(verificationRepository::delete);
    }
    
    // 비밀번호 재설정 관련 메서드는 유사하게 구현
}
```

---

## ✅ 테스트 전략

### 1. 단위 테스트

#### VerificationCodeGeneratorTest
- 6자리 숫자 생성 검증
- 범위 검증 (000000 ~ 999999)
- 고유성 검증 (10000개 생성 시 중복 없음)

#### EmailVerificationServiceImplTest
- 인증 코드 발송 성공
- 중복 이메일 발송 실패 (회원가입)
- 재전송 쿨다운 검증
- 코드 검증 성공
- 코드 불일치 검증
- 만료된 코드 검증
- 최대 시도 횟수 초과 검증

#### EmailSendServiceImplTest (Mock)
- JavaMailSender Mock으로 발송 검증
- 예외 발생 시 EmailSendException 변환

### 2. 통합 테스트

#### EmailVerificationIntegrationTest
- 회원가입 플로우: 코드 발송 → 검증 → 회원가입
- 비밀번호 재설정 플로우: 코드 발송 → 검증 → 비밀번호 변경
- 만료 시나리오 (시간 조작)
- 재전송 시나리오

---

## 🚀 배포 고려사항

### 1. Gmail 앱 비밀번호 생성
- Google 계정 2단계 인증 활성화 필수
- 앱 비밀번호 생성: https://myaccount.google.com/apppasswords
- `.env` 파일에 `MAIL_APP_PASSWORD` 설정

### 2. 프로덕션 환경
- **SMTP 타임아웃**: 이미 `application.properties`에 설정됨 (5초)
- **발송 로그**: 성공/실패 이력을 로그에 기록 (PII 제외)
  ```java
  // EmailSendServiceImpl
  try {
      mailSender.send(message);
      log.info("이메일 발송 성공 - type: {}", type);
  } catch (MessagingException e) {
      log.error("이메일 발송 실패 - type: {}, error: {}", type, e.getMessage());
      throw new EmailSendException("이메일 발송에 실패했습니다", e);
  }
  ```
  
  **⚠️ PII 보호**: 수신자 이메일 주소(`to`)는 개인정보이므로 로그에 기록하지 않습니다.

### 3. 배치 작업 (필수)
만료된 인증 데이터를 자동으로 정리하여 DB 용량 관리:

```java
@Component
@RequiredArgsConstructor
public class EmailVerificationCleanupScheduler {
    
    private final EmailVerificationRepository verificationRepository;
    
    @Scheduled(cron = "0 0 2 * * ?") // 매일 새벽 2시
    @Transactional
    public void cleanupExpiredVerifications() {
        LocalDateTime threshold = LocalDateTime.now().minusHours(24);
        
        // 인증 완료 후 24시간 지난 데이터 삭제
        verificationRepository.deleteByExpiresAtBeforeAndVerifiedTrue(threshold);
        
        // 미인증 상태로 24시간 지난 데이터 삭제
        verificationRepository.deleteByCreatedAtBeforeAndVerifiedFalse(threshold);
        
        log.info("만료된 이메일 인증 데이터 정리 완료");
    }
}
```

**설정 추가 (`Application.java` 또는 별도 Config)**:
```java
@EnableScheduling
@SpringBootApplication
public class Application {
    // ...
}
```

---

## 📌 참고사항

### 데이터 저장 방식
이메일 인증 코드는 **DB(`email_verification` 테이블)**에만 저장합니다:
- **이유:** 감사 추적, 법적 보호, 디버깅 용이성
- **Redis 미사용:** 인증 데이터는 영속성이 중요하므로 DB만 사용

### 회원가입 플로우 변경
기존 `SignupService`에 이메일 인증 여부 확인 로직 추가 필요:
```java
// SignupServiceImpl.java
public AccountResponse signup(SignupRequest request) {
    // 이메일 인증 완료 여부 확인
    boolean verified = verificationRepository.existsByEmailAndTypeAndVerifiedTrue(
        request.getEmail(), 
        VerificationType.SIGNUP
    );
    
    if (!verified) {
        throw new EmailNotVerifiedException("이메일 인증이 필요합니다");
    }
    
    // 기존 회원가입 로직...
}
```

---

## 📅 구현 일정 (예상)

1. **Phase 1 (1일):** 의존성 추가, 설정, 엔티티/DTO 생성
2. **Phase 2 (1일):** Repository, Util, 이메일 템플릿
3. **Phase 3 (2일):** Service 구현 (발송, 검증)
4. **Phase 4 (1일):** Controller, 예외 처리
5. **Phase 5 (2일):** 단위 테스트, 통합 테스트
6. **Phase 6 (1일):** 회원가입/비밀번호 재설정 통합, 검증

**총 예상 기간:** 7-8일

---

## ✨ 추가 개선 사항 (향후)

1. **이메일 템플릿 엔진:** Thymeleaf로 동적 HTML 생성
2. **국제화(i18n):** 다국어 지원 (한국어/영어)
3. **알림 설정:** 사용자가 이메일 알림 수신 여부 선택
4. **OAuth 인증:** Google/Kakao 로그인 시 이메일 인증 생략
5. **SMS 인증:** 이메일 대신 문자 인증 옵션 제공

---

**작성일:** 2025년 1월 15일  
**작성자:** 태윤  
**문서 버전:** 1.0
