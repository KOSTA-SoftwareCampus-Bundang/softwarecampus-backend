# 비밀번호 재설정 기능 - 전략 설계

## 📋 개요

**목적:** 로그인한 사용자가 이메일 인증을 통해 안전하게 비밀번호를 재설정할 수 있는 기능 제공  
**방식:** 이메일 인증 코드 직접 검증 + 비밀번호 변경 (1단계 처리)  
**보안:** 이메일 인증 기반, BCrypt 암호화, 비밀번호 복잡도 검증  
**인증:** JWT 토큰 (마이페이지) + 이메일 6자리 인증 코드  
**선행 기능:** SMTP 이메일 인증 (update/smtp/전략_설계.md)

---

## 🎯 구현 범위

### 1. 기능 시나리오

#### 비밀번호 재설정 플로우
```
1. 사용자 로그인 (JWT 토큰 발급)
2. 마이페이지 → "비밀번호 변경" 버튼 클릭
3. POST /api/auth/email/send-reset-code
   → 현재 로그인한 이메일로 6자리 코드 발송
4. 사용자가 이메일에서 받은 코드 입력
5. 새 비밀번호 입력 (프론트엔드에서 2번 입력 후 일치 검증)
6. PUT /api/mypage/password
   Request: {
     "code": "123456",
     "newPassword": "newPassword456@"
   }
   → 코드 검증 (EmailVerification 조회)
   → 비밀번호 복잡도 검증
   → BCrypt 암호화 후 DB 업데이트
   → 인증 데이터 삭제 (1회용)
7. 응답: 200 OK
8. (옵션) 비밀번호 변경 완료 알림 이메일 발송
```

**✨ 핵심 포인트:**
- 코드 검증과 비밀번호 변경을 **한 번의 API 호출**로 처리
- `confirmPassword` 검증은 **프론트엔드**에서 처리 (즉각적인 UX 피드백)
- 백엔드는 `code` + `newPassword`만 받아서 처리

---

## 🏗️ 아키텍처 설계

### 1. 기존 리소스 활용 및 의존성

#### SMTP 이메일 인증 기능 (선행 필수)
- ✅ `EmailVerificationService` - 인증 코드 발송
- ✅ `EmailVerification` 엔티티 (type: PASSWORD_RESET, verified: false)
- ✅ `VerificationType.PASSWORD_RESET` Enum
- ✅ `POST /api/auth/email/send-reset-code` - 코드 발송 API
- ✅ `EmailVerificationRepository` - 코드 조회/검증

**⚠️ 중요:** SMTP 이메일 인증 기능이 먼저 구현되어 있어야 합니다.  
참고: `update/smtp/전략_설계.md`

#### 이미 존재하는 마이페이지 컴포넌트
- ✅ `MyPageController` (`/api/mypage`)
- ✅ `ProfileService` 인터페이스
- ✅ `ProfileServiceImpl` (프로필 조회/수정/삭제)
- ✅ `AccountRepository`
- ✅ `Account` 엔티티 (`password` 필드)
- ✅ `PasswordEncoder` (Spring Security - BCrypt)
- ✅ JWT 인증 (@AuthenticationPrincipal)

#### 새로 추가할 컴포넌트
- 🆕 `ResetPasswordRequest` DTO (code + newPassword)
- 🆕 `ProfileService.resetPassword()` 메서드
- 🆕 `MyPageController.resetPassword()` 엔드포인트

**⚠️ 주의:** 중간 토큰(resetToken) 생성/검증 로직 **불필요**

---

### 2. 데이터 모델

#### Account 엔티티 (기존)
```java
@Entity
public class Account extends BaseSoftDeleteSupportEntity {
    
    @Column(nullable = false)
    private String email;
    
    @Column(nullable = false)
    private String password;  // BCrypt 암호화된 비밀번호
    
    // ... 기타 필드
}
```

**변경사항 없음** - 기존 `password` 필드만 업데이트

---

#### EmailVerification 엔티티 (SMTP 기능에서 제공)
```java
@Entity
@Table(name = "email_verification")
public class EmailVerification {
    
    @Column(nullable = false)
    private String email;
    
    @Column(nullable = false, length = 6)
    private String code;  // 6자리 숫자 코드
    
    @Enumerated(EnumType.STRING)
    @Column(nullable = false)
    private VerificationType type;  // PASSWORD_RESET
    
    @Column(nullable = false)
    private boolean verified = false;  // 아직 사용 안 함
    
    @Column(nullable = false)
    private LocalDateTime expiresAt;  // 3분 후
    
    @Column(nullable = false)
    private int attempts = 0;  // 시도 횟수 (최대 5회)
}
```

**활용 방식:**
- `type = PASSWORD_RESET`으로 비밀번호 재설정 전용 코드 관리
- 코드 검증 성공 시 `verified = true` 설정 후 비밀번호 변경
- 사용 후 레코드 삭제하여 1회용 보장

---

### 3. 레이어 구조

#### 패키지 구조
```
src/main/java/com/softwarecampus/backend/
├── controller/user/
│   ├── EmailVerificationController.java  ← SMTP 기능 (이미 구현)
│   └── MyPageController.java  ← resetPassword() 엔드포인트 추가
├── service/user/
│   ├── email/
│   │   ├── EmailVerificationService.java  ← SMTP 기능 (이미 구현)
│   │   └── EmailVerificationServiceImpl.java
│   └── profile/
│       ├── ProfileService.java  ← resetPassword() 메서드 선언 추가
│       └── ProfileServiceImpl.java  ← 비밀번호 재설정 로직 구현
├── dto/user/
│   └── ResetPasswordRequest.java  (신규)
├── repository/user/
│   ├── AccountRepository.java  (기존)
│   └── EmailVerificationRepository.java  (SMTP 기능 - 기존)
└── domain/user/
    ├── Account.java  (기존)
    └── EmailVerification.java  (SMTP 기능 - 기존)
```

---

## 📝 API 명세

### 비밀번호 재설정 (이메일 코드 직접 검증)

**Endpoint:** `PUT /api/mypage/password`

**인증:** JWT 토큰 필수 (Authorization: Bearer {token})

**선행 조건:**
1. `POST /api/auth/email/send-reset-code` 호출 (이메일 인증 코드 발송)

**Request:**
```json
{
  "code": "123456",
  "newPassword": "newPassword456@"
}
```

**Request 필드 설명:**
| 필드 | 타입 | 필수 | 설명 | 제약조건 |
|------|------|------|------|----------|
| code | String | ✅ | 이메일로 받은 6자리 인증 코드 | 6자리 숫자, 3분 유효 |
| newPassword | String | ✅ | 새 비밀번호 | 8~20자, 영문+숫자+특수문자 포함 |

**⚠️ 참고:**
- `confirmPassword` (비밀번호 확인) 검증은 **프론트엔드에서 처리**
- 백엔드는 `code`와 `newPassword`만 검증

---

**Response (200 OK):**
```
(응답 본문 없음 - 빈 body)
```

> **참고:** 실제 구현에서는 `ResponseEntity.ok().build()`를 사용하여 200 OK와 빈 body를 반환합니다.

---

**Error Cases:**

**400 Bad Request - Validation 실패**
```json
{
  "type": "https://api.softwarecampus.com/problems/validation-error",
  "title": "Validation Failed",
  "status": 400,
  "detail": "요청 본문에 유효하지 않은 필드가 있습니다.",
  "instance": "/api/mypage/password",
  "errors": {
    "code": "인증 코드를 입력해주세요",
    "newPassword": "비밀번호는 8~50자, 대소문자, 숫자, 특수문자를 포함해야 합니다"
  }
}
```

**400 Bad Request - 인증 코드 불일치**
```json
{
  "type": "https://api.softwarecampus.com/problems/invalid-verification-code",
  "title": "Invalid Verification Code",
  "status": 400,
  "detail": "인증 코드가 일치하지 않습니다.",
  "instance": "/api/mypage/password"
}
```

**400 Bad Request - 인증 코드 만료**
```json
{
  "type": "https://api.softwarecampus.com/problems/expired-verification-code",
  "title": "Expired Verification Code",
  "status": 400,
  "detail": "인증 코드가 만료되었습니다. 다시 이메일 인증을 진행해주세요.",
  "instance": "/api/mypage/password"
}
```

**429 Too Many Requests - 최대 시도 횟수 초과**
```json
{
  "type": "https://api.softwarecampus.com/problems/max-attempts-exceeded",
  "title": "Max Attempts Exceeded",
  "status": 429,
  "detail": "최대 시도 횟수를 초과했습니다. 30분 후에 다시 시도하세요.",
  "instance": "/api/mypage/password"
}
```

**401 Unauthorized - 인증 실패**
```json
{
  "timestamp": "2025-11-29T12:00:00.000+00:00",
  "status": 401,
  "error": "Unauthorized",
  "message": "Full authentication is required to access this resource",
  "path": "/api/mypage/password"
}
```

**404 Not Found - 계정 없음**
```json
{
  "type": "https://api.softwarecampus.com/problems/account-not-found",
  "title": "Account Not Found",
  "status": 404,
  "detail": "계정을 찾을 수 없습니다.",
  "instance": "/api/mypage/password"
}
```

**404 Not Found - 인증 요청 없음**
```json
{
  "type": "https://api.softwarecampus.com/problems/verification-not-found",
  "title": "Verification Not Found",
  "status": 404,
  "detail": "인증 요청을 찾을 수 없습니다. 먼저 이메일 인증 코드를 요청해주세요.",
  "instance": "/api/mypage/password"
}
```

---

## 🔐 보안 정책

### 1. 이메일 인증 기반 보안 (SMTP 기능 활용)
- **인증 코드:** 6자리 숫자, 3분 유효
- **최대 시도:** 5회 (초과 시 30분 차단)
- **재전송 제한:** 60초 쿨다운
- **1회용 보장:** 사용 후 EmailVerification 레코드 삭제

### 2. 비밀번호 암호화
- **알고리즘:** BCrypt (Spring Security 기본)
- **강도:** strength = 10 (2^10 rounds)
- **Salt:** BCrypt 자체 제공 (자동 생성)

### 3. 비밀번호 복잡도 규칙
- **최소 길이:** 8자
- **최대 길이:** 20자
- **필수 포함:**
  - 영문자 1개 이상 (A-Za-z)
  - 숫자 1개 이상 (0-9)
  - 특수문자 1개 이상 (!@#$%^&*()-_=+[]{}|;:'",.<>/?~)

**정규식:**
```regex
^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()\-_=+\[\]{}|;:'",.<>/?~])[A-Za-z\d!@#$%^&*()\-_=+\[\]{}|;:'",.<>/?~]{8,20}$
```

### 4. 검증 순서
1. **JWT 토큰 검증:** 로그인 상태 확인 (마이페이지 접근 권한)
2. **입력 검증:** `@Valid` 어노테이션으로 DTO 필드 검증
3. **이메일 인증 코드 검증:**
   - EmailVerification 조회 (email + type + verified=false)
   - 만료 시간 확인 (expiresAt)
   - 시도 횟수 확인 (attempts < 5)
   - 코드 일치 확인
4. **복잡도 검증:** 정규식으로 비밀번호 강도 확인
5. **암호화 및 저장:** `PasswordEncoder.encode(newPassword)`
6. **인증 데이터 삭제:** EmailVerification 레코드 삭제 (재사용 방지)

### 5. 보안 고려사항
- ❌ **비밀번호 평문 로깅 금지**
- ❌ **인증 코드 로깅 금지**
- ✅ **이메일 인증 2단계** (코드 발송 → 코드 검증 + 비밀번호 변경)
- ✅ **이메일 알림 권장** (비밀번호 변경 성공 시)
- ✅ **Redis 캐시 무효화** (`userDetails:{email}` 캐시 삭제)
- ✅ **트랜잭션 처리** (`@Transactional` 보장)
- ✅ **1회용 코드** (사용 후 삭제)

---

## 🛠️ 구현 상세

### 1. ResetPasswordRequest DTO

```java
package com.softwarecampus.backend.dto.user;

import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.Pattern;
import jakarta.validation.constraints.Size;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;

/**
 * 비밀번호 재설정 요청 DTO (이메일 인증 코드 기반)
 * 
 * 사용 시나리오:
 * 1. 사용자가 이메일 인증 코드를 받음 (POST /api/auth/email/send-reset-code)
 * 2. 코드와 새 비밀번호를 함께 전송하여 비밀번호 재설정 (PUT /api/mypage/password)
 * 
 * 참고:
 * - confirmPassword 검증은 프론트엔드에서 처리
 * - 백엔드는 code + newPassword만 받아서 처리
 */
@Getter
@NoArgsConstructor
@AllArgsConstructor
public class ResetPasswordRequest {
    
    @NotBlank(message = "인증 코드를 입력해주세요")
    @Size(min = 6, max = 6, message = "인증 코드는 6자리여야 합니다")
    @Pattern(regexp = "^[0-9]{6}$", message = "인증 코드는 6자리 숫자여야 합니다")
    private String code;
    
    @NotBlank(message = "새 비밀번호를 입력해주세요")
    @Size(min = 8, max = 20, message = "비밀번호는 8~20자여야 합니다")
    @Pattern(
        regexp = "^(?=.*[A-Za-z])(?=.*\\d)(?=.*[!@#$%^&*()\\-_=+\\[\\]{}|;:'\",.<>/?~])[A-Za-z\\d!@#$%^&*()\\-_=+\\[\\]{}|;:'\",.<>/?~]{8,20}$",
        message = "비밀번호는 영문, 숫자, 특수문자를 포함해야 합니다"
    )
    private String newPassword;
}
```

---

### 2. ProfileService (인터페이스 확장)

```java
package com.softwarecampus.backend.service.user.profile;

import com.softwarecampus.backend.dto.user.ResetPasswordRequest;

/**
 * 계정 조회 및 프로필 관리 Service 인터페이스
 */
public interface ProfileService {
    
    // ... 기존 메서드들 (getAccountByEmail, updateProfile, deleteAccount)
    
    /**
     * 비밀번호 재설정 (이메일 인증 코드 기반)
     * 
     * @param email 로그인한 사용자 이메일 (JWT에서 추출)
     * @param request 비밀번호 재설정 요청 정보 (code + newPassword)
     * @throws com.softwarecampus.backend.exception.user.AccountNotFoundException 계정이 존재하지 않는 경우
     * @throws com.softwarecampus.backend.exception.user.VerificationNotFoundException 인증 요청을 찾을 수 없음
     * @throws com.softwarecampus.backend.exception.user.InvalidVerificationCodeException 코드 불일치
     * @throws com.softwarecampus.backend.exception.user.VerificationCodeExpiredException 코드 만료
     * @throws com.softwarecampus.backend.exception.user.MaxAttemptsExceededException 최대 시도 횟수 초과
     */
    void resetPassword(String email, ResetPasswordRequest request);
}
```

---

### 3. ProfileServiceImpl (구현)

```java
package com.softwarecampus.backend.service.user.profile;

import com.softwarecampus.backend.domain.common.VerificationType;
import com.softwarecampus.backend.domain.user.Account;
import com.softwarecampus.backend.dto.user.AccountResponse;
import com.softwarecampus.backend.dto.user.EmailVerificationCodeRequest;
import com.softwarecampus.backend.dto.user.ResetPasswordRequest;
import com.softwarecampus.backend.dto.user.UpdateProfileRequest;
import com.softwarecampus.backend.exception.user.AccountNotFoundException;
import com.softwarecampus.backend.repository.user.AccountRepository;
import com.softwarecampus.backend.repository.user.EmailVerificationRepository;
import com.softwarecampus.backend.service.user.email.EmailVerificationService;
import com.softwarecampus.backend.util.EmailUtils;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.security.crypto.password.PasswordEncoder;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

/**
 * 계정 조회 및 프로필 관리 Service 구현체
 * - Phase 5: 기본 조회 기능
 * - Phase 15-1: 프로필 수정/삭제 기능 추가
 * - Phase 15-2: 비밀번호 재설정 기능 추가
 */
@Slf4j
@Service
@RequiredArgsConstructor
@Transactional(readOnly = true)
public class ProfileServiceImpl implements ProfileService {
    
    private final AccountRepository accountRepository;
    private final EmailVerificationRepository emailVerificationRepository;
    private final EmailVerificationService emailVerificationService;
    private final PasswordEncoder passwordEncoder;
    
    // ... 기존 메서드들
    
    /**
     * 비밀번호 재설정 (이메일 인증 코드 검증)
     * - EmailVerificationService.verifyResetCode() 재사용
     */
    @Override
    @Transactional
    public void resetPassword(String email, ResetPasswordRequest request) {
        // 1. 입력 검증
        validateEmailInput(email);
        
        log.info("비밀번호 재설정 시도: email={}", EmailUtils.maskEmail(email));
        
        // 2. 계정 조회
        Account account = accountRepository.findByEmail(email)
            .orElseThrow(() -> new AccountNotFoundException("계정을 찾을 수 없습니다."));
        
        // 3. 이메일 인증 코드 검증 (EmailVerificationService 재사용)
        // - 차단 상태 확인, 코드 만료 확인, 코드 일치 검증, 시도 횟수 관리 등 모두 포함
        EmailVerificationCodeRequest codeRequest = new EmailVerificationCodeRequest(email, request.getCode());
        emailVerificationService.verifyResetCode(codeRequest);
        
        // 4. 비밀번호 변경
        String encodedPassword = passwordEncoder.encode(request.getNewPassword());
        account.setPassword(encodedPassword);
        
        // 5. 인증 레코드 삭제 (일회용 보장)
        emailVerificationRepository.deleteByEmailAndType(email, VerificationType.PASSWORD_RESET);
        
        log.info("비밀번호 재설정 완료: email={}, accountId={}", 
            EmailUtils.maskEmail(email), account.getId());
    }
}
```

> **참고:** 실제 구현에서는 `EmailVerificationService.verifyResetCode()`를 재사용하여 코드 검증 로직 중복을 제거했습니다.

---

### 4. EmailVerificationRepository (메서드 확인)

```java
package com.softwarecampus.backend.repository.user;

import com.softwarecampus.backend.domain.user.EmailVerification;
import com.softwarecampus.backend.domain.common.VerificationType;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.Optional;

public interface EmailVerificationRepository extends JpaRepository<EmailVerification, Long> {
    
    /**
     * 미인증 상태의 이메일 인증 레코드 조회
     * (SMTP 기능에서 이미 제공)
     */
    Optional<EmailVerification> findByEmailAndTypeAndVerifiedFalse(
        String email,
        VerificationType type
    );
    
    // ... 기타 SMTP 기능 메서드들
}
```

---

### 5. MyPageController (엔드포인트 추가)

```java
package com.softwarecampus.backend.controller.user;

import com.softwarecampus.backend.dto.user.AccountResponse;
import com.softwarecampus.backend.dto.user.ResetPasswordRequest;
import com.softwarecampus.backend.dto.user.UpdateProfileRequest;
import com.softwarecampus.backend.service.user.profile.ProfileService;
import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.http.ResponseEntity;
import org.springframework.security.core.annotation.AuthenticationPrincipal;
import org.springframework.security.core.userdetails.UserDetails;
import org.springframework.web.bind.annotation.*;

/**
 * 마이페이지 API Controller
 * 
 * 엔드포인트:
 * - GET /api/mypage/profile: 프로필 조회
 * - PATCH /api/mypage/profile: 프로필 수정
 * - DELETE /api/mypage/account: 계정 삭제
 * - PUT /api/mypage/password: 비밀번호 재설정 (이메일 인증 코드 기반)
 * 
 * 인증: 모든 엔드포인트 JWT 토큰 필수
 */
@Slf4j
@RestController
@RequestMapping("/api/mypage")
@RequiredArgsConstructor
public class MyPageController {

    private final ProfileService profileService;

    // ... 기존 엔드포인트들 (getProfile, updateProfile, deleteAccount)
    
    /**
     * 비밀번호 재설정 (이메일 인증 코드 기반)
     * 
     * 선행 조건:
     * 1. POST /api/auth/email/send-reset-code (인증 코드 발송)
     * 
     * @param userDetails Spring Security 인증 정보
     * @param request 비밀번호 재설정 요청 정보 (code + newPassword)
     * @return 200 OK
     */
    @PutMapping("/password")
    public ResponseEntity<Void> resetPassword(
            @AuthenticationPrincipal UserDetails userDetails,
            @Valid @RequestBody ResetPasswordRequest request) {
        
        String email = userDetails.getUsername();
        log.info("비밀번호 재설정 요청");
        
        profileService.resetPassword(email, request);
        
        return ResponseEntity.ok().build();
    }
}
```

---

## ✅ 테스트 전략

### 1. 단위 테스트

#### ProfileServiceImplTest
```java
@Test
void resetPassword_Success() {
    // Given
    String email = "user@example.com";
    String code = "123456";
    ResetPasswordRequest request = new ResetPasswordRequest(code, "newPassword456@");
    
    EmailVerification verification = EmailVerification.builder()
        .email(email)
        .code(code)
        .type(VerificationType.PASSWORD_RESET)
        .expiresAt(LocalDateTime.now().plusMinutes(3))
        .verified(false)
        .attempts(0)
        .build();
    
    Account account = Account.builder()
        .email(email)
        .password(passwordEncoder.encode("oldPassword123!"))
        .build();
    
    when(verificationRepository.findByEmailAndTypeAndVerifiedFalse(email, VerificationType.PASSWORD_RESET))
        .thenReturn(Optional.of(verification));
    when(accountRepository.findByEmail(email)).thenReturn(Optional.of(account));
    
    // When
    profileService.resetPassword(email, request);
    
    // Then
    assertTrue(passwordEncoder.matches("newPassword456@", account.getPassword()));
    verify(verificationRepository).delete(verification);  // 1회용 보장
}

@Test
void resetPassword_InvalidCode_ThrowsException() {
    // Given
    String email = "user@example.com";
    ResetPasswordRequest request = new ResetPasswordRequest("999999", "newPassword456@");
    
    EmailVerification verification = EmailVerification.builder()
        .email(email)
        .code("123456")
        .type(VerificationType.PASSWORD_RESET)
        .expiresAt(LocalDateTime.now().plusMinutes(3))
        .verified(false)
        .attempts(0)
        .build();
    
    when(verificationRepository.findByEmailAndTypeAndVerifiedFalse(email, VerificationType.PASSWORD_RESET))
        .thenReturn(Optional.of(verification));
    
    // When & Then
    assertThrows(InvalidVerificationCodeException.class, 
        () -> profileService.resetPassword(email, request));
    
    // 시도 횟수 증가 확인
    assertEquals(1, verification.getAttempts());
    verify(verificationRepository).save(verification);
}

@Test
void resetPassword_ExpiredCode_ThrowsException() {
    // Given
    String email = "user@example.com";
    ResetPasswordRequest request = new ResetPasswordRequest("123456", "newPassword456@");
    
    EmailVerification verification = EmailVerification.builder()
        .email(email)
        .code("123456")
        .type(VerificationType.PASSWORD_RESET)
        .expiresAt(LocalDateTime.now().minusMinutes(1))  // 만료됨
        .verified(false)
        .attempts(0)
        .build();
    
    when(verificationRepository.findByEmailAndTypeAndVerifiedFalse(email, VerificationType.PASSWORD_RESET))
        .thenReturn(Optional.of(verification));
    
    // When & Then
    assertThrows(VerificationCodeExpiredException.class, 
        () -> profileService.resetPassword(email, request));
}

@Test
void resetPassword_MaxAttemptsExceeded_ThrowsException() {
    // Given
    String email = "user@example.com";
    ResetPasswordRequest request = new ResetPasswordRequest("999999", "newPassword456@");
    
    EmailVerification verification = EmailVerification.builder()
        .email(email)
        .code("123456")
        .type(VerificationType.PASSWORD_RESET)
        .expiresAt(LocalDateTime.now().plusMinutes(3))
        .verified(false)
        .attempts(5)  // 이미 5회 시도
        .build();
    
    when(verificationRepository.findByEmailAndTypeAndVerifiedFalse(email, VerificationType.PASSWORD_RESET))
        .thenReturn(Optional.of(verification));
    
    // When & Then
    assertThrows(MaxAttemptsExceededException.class, 
        () -> profileService.resetPassword(email, request));
    
    // 차단 설정 확인
    assertNotNull(verification.getBlockedUntil());
    verify(verificationRepository).save(verification);
}
```

---

### 2. 통합 테스트

#### MyPageControllerIntegrationTest
```java
@Test
@WithMockUser(username = "user@example.com")
void resetPassword_Success() throws Exception {
    // Given
    ResetPasswordRequest request = new ResetPasswordRequest("123456", "newPassword456@");
    
    // When & Then
    mockMvc.perform(put("/api/mypage/password")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk());
}

@Test
void resetPassword_NoAuth_Returns401() throws Exception {
    // Given
    ResetPasswordRequest request = new ResetPasswordRequest("123456", "newPassword456@");
    
    // When & Then
    mockMvc.perform(put("/api/mypage/password")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isUnauthorized());
}

@Test
@WithMockUser(username = "user@example.com")
void resetPassword_InvalidCode_Returns400() throws Exception {
    // Given
    ResetPasswordRequest request = new ResetPasswordRequest("999999", "newPassword456@");
    
    // When & Then
    mockMvc.perform(put("/api/mypage/password")
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.detail").value("인증 코드가 일치하지 않습니다."));
}
```

---

## 🚀 배포 고려사항

### 1. 보안 체크리스트
- [x] 비밀번호 평문 로깅 금지
- [x] BCrypt 암호화 적용
- [x] 복잡도 검증 (정규식)
- [x] 이메일 인증 코드 검증
- [x] JWT 인증 필수
- [x] 1회용 코드 (사용 후 삭제)
- [ ] (옵션) 비밀번호 변경 알림 이메일

### 2. 데이터베이스
- **변경사항 없음** - 기존 `account.password` 컬럼 활용
- **마이그레이션 불필요**
- EmailVerification 테이블은 SMTP 기능에서 이미 생성됨

### 3. Redis 캐시 무효화 (권장)
프로필 수정과 유사하게, 비밀번호 변경 시 사용자 캐시를 삭제하여 최신 정보 반영:

```java
// ProfileServiceImpl.java (옵션)
@Override
@Transactional
public void resetPassword(String email, ResetPasswordRequest request) {
    // ... 비밀번호 변경 로직
    
    // Redis 캐시 무효화
    String cacheKey = "userDetails:" + email;
    redisTemplate.delete(cacheKey);
    
    log.info("비밀번호 변경 완료 및 캐시 무효화: email={}", EmailUtils.maskEmail(email));
}
```

### 4. 알림 이메일 발송 (권장)
비밀번호 변경 성공 시 보안을 위해 알림 이메일 발송:

```java
// ProfileServiceImpl.java (옵션)
@Override
@Transactional
public void resetPassword(String email, ResetPasswordRequest request) {
    // ... 비밀번호 변경 로직
    
    // 알림 이메일 발송
    emailService.sendPasswordChangedNotification(email);
    
    log.info("비밀번호 변경 알림 이메일 발송: email={}", EmailUtils.maskEmail(email));
}
```

---

## 📌 프론트엔드 연동 가이드

### 1. API 호출 예시 (Axios)

```typescript
// src/services/mypage.ts
import axios from 'axios';

export interface ResetPasswordRequest {
  code: string;
  newPassword: string;
}

export const resetPassword = async (data: ResetPasswordRequest): Promise<void> => {
  const response = await axios.put('/api/mypage/password', data, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`
    }
  });
  return response.data;
};
```

### 2. React Hook 사용 예시

```typescript
// src/pages/MyPage/useMyPage.ts
import { useMutation } from '@tanstack/react-query';
import { resetPassword, ResetPasswordRequest } from '@/services/mypage';

export const useResetPassword = () => {
  return useMutation({
    mutationFn: (data: ResetPasswordRequest) => resetPassword(data),
    onSuccess: () => {
      alert('비밀번호가 성공적으로 변경되었습니다.');
    },
    onError: (error: any) => {
      if (error.response?.status === 400) {
        alert(error.response.data.detail || '비밀번호 변경에 실패했습니다.');
      } else if (error.response?.status === 429) {
        alert('최대 시도 횟수를 초과했습니다. 30분 후에 다시 시도하세요.');
      }
    }
  });
};
```

### 3. 프론트엔드 검증 예시

```typescript
// src/pages/MyPage/components/PasswordChangeModal.tsx
const [newPassword, setNewPassword] = useState('');
const [confirmPassword, setConfirmPassword] = useState('');
const [passwordError, setPasswordError] = useState('');

const handlePasswordChange = () => {
  // 프론트엔드에서 confirmPassword 검증
  if (newPassword !== confirmPassword) {
    setPasswordError('새 비밀번호와 확인 비밀번호가 일치하지 않습니다.');
    return;
  }
  
  // 복잡도 검증 (옵션 - 백엔드에서도 검증함)
  const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()\-_=+\[\]{}|;:'",.<>/?~])[A-Za-z\d!@#$%^&*()\-_=+\[\]{}|;:'",.<>/?~]{8,20}$/;
  if (!passwordRegex.test(newPassword)) {
    setPasswordError('비밀번호는 8~20자, 영문, 숫자, 특수문자를 포함해야 합니다.');
    return;
  }
  
  // 백엔드 API 호출 (code + newPassword만 전송)
  resetPasswordMutation.mutate({
    code,
    newPassword
  });
};
```

---

## 📅 구현 일정 (예상)

### Phase 1: DTO 생성 (0.5일)
- ResetPasswordRequest DTO 작성
- Validation 어노테이션 추가

### Phase 2: Service 로직 구현 (1일)
- ProfileService.resetPassword() 메서드 선언
- ProfileServiceImpl.resetPassword() 구현
- 코드 검증 → 비밀번호 변경 → 인증 데이터 삭제 로직

### Phase 3: Controller 엔드포인트 추가 (0.5일)
- MyPageController.resetPassword() 엔드포인트 추가
- 요청/응답 처리

### Phase 4: 단위 테스트 작성 (1일)
- ProfileServiceImplTest (성공/실패 케이스)
- 코드 불일치, 만료, 최대 시도 초과 테스트

### Phase 5: 통합 테스트 작성 (1일)
- MyPageControllerIntegrationTest
- 전체 플로우 테스트 (이메일 인증 → 비밀번호 재설정)

### Phase 6: 프론트엔드 연동 테스트 (0.5일)
- 이메일 인증 UI → 비밀번호 재설정 UI 연동
- 에러 핸들링 확인

**총 예상 기간:** 4-5일

**⚠️ 선행 조건:** SMTP 이메일 인증 기능이 먼저 구현되어 있어야 합니다.  
참고: `update/smtp/전략_설계.md`

---

## ✨ 추가 개선 사항 (향후)

1. **이메일 알림 자동화:** 비밀번호 변경 성공 시 자동 알림 이메일 발송
2. **비밀번호 변경 이력 저장:** 감사 추적을 위해 변경 시각 기록
3. **비밀번호 재사용 제한 확대:** 최근 5개 비밀번호 재사용 방지
4. **2단계 인증 (2FA) 강화:** SMS 인증 추가 옵션
5. **비밀번호 만료 정책:** 90일마다 비밀번호 변경 권장
6. **다중 디바이스 로그아웃:** 비밀번호 변경 시 다른 기기 세션 무효화

---

**작성일:** 2025년 11월 29일  
**작성자:** GitHub Copilot  
**문서 버전:** 3.0 (단순화 버전 - 코드 직접 검증 방식)  
**참고 문서:** 
- SMTP 전략 설계 (update/smtp/전략_설계.md)
- 마이페이지 API 명세 (docs/api/03_mypage.md)
