# 조회수 중복 방지 기능

## 📋 개요

| 항목 | 내용 |
|------|------|
| **요청일** | 2025-12-02 |
| **완료일** | 2025-12-02 |
| **상태** | ✅ 완료 |
| **우선순위** | 중간 |
| **영향 범위** | 백엔드 (Board 모듈) |

---

## 🔍 이전 상태 (문제점)

```java
// BoardServiceImpl.java - getBoardById() [수정 전]
board.setHits(board.getHits() + 1);
```

- 게시글 상세 조회 시 **무조건 조회수 +1**
- 새로고침해도 증가
- 작성자 본인이 봐도 증가
- 같은 사용자가 여러 번 봐도 매번 증가

---

## ✅ 구현된 기능

### 조회수 증가 조건

| 사용자 유형 | 조회수 증가 조건 |
|------------|-----------------|
| **작성자 본인** | ❌ 증가 안함 |
| **로그인 사용자** | 오늘 첫 조회일 때만 +1 (account_id 기준) |
| **비로그인 사용자** | 오늘 첫 조회일 때만 +1 (IP 기준) |

### 하루 기준
- 자정(00:00) 기준 리셋

---

## 🤔 설계 결정 사항

| # | 항목 | 결정 | 사유 |
|---|------|------|------|
| 1 | Service에 Request 전달 | ❌ IP를 String으로 전달 | Service 레이어 HTTP 의존성 제거, 테스트 용이 |
| 2 | 동시성 중복 | 무시 | 드문 케이스, 비용 대비 효과 낮음 |
| 3 | 공유 IP 문제 | 감수 | 비로그인 사용자 정확도는 낮아도 OK |
| 4 | 테이블 크기 | 30일 후 삭제 | 스케줄러로 자동 정리 |
| 5 | 기존 조회수 | 유지 | 데이터 손실 방지 |
| 6 | 비로그인→로그인 전환 | 허용 | 어차피 하루 2번, 복잡도 대비 효과 낮음 |

---

## 🗄️ DB 설계

### 신규 테이블: `board_view`

```sql
CREATE TABLE board_view (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    board_id BIGINT NOT NULL,
    account_id BIGINT NULL,           -- 로그인 사용자 (nullable)
    ip_address VARCHAR(45) NULL,      -- 비로그인 사용자용 (IPv6 지원)
    viewed_at DATETIME NOT NULL,
    
    CONSTRAINT fk_board_view_board 
        FOREIGN KEY (board_id) REFERENCES board(id) ON DELETE CASCADE,
    CONSTRAINT fk_board_view_account 
        FOREIGN KEY (account_id) REFERENCES account(id) ON DELETE CASCADE,
    
    -- 중복 방지 및 조회 성능용 인덱스
    INDEX idx_board_view_account (board_id, account_id, viewed_at),
    INDEX idx_board_view_ip (board_id, ip_address, viewed_at)
);
```

### 필드 설명

| 필드 | 타입 | 설명 |
|------|------|------|
| `id` | BIGINT | PK, Auto Increment |
| `board_id` | BIGINT | 게시글 FK (NOT NULL) |
| `account_id` | BIGINT | 로그인 사용자 FK (NULL 가능) |
| `ip_address` | VARCHAR(45) | 비로그인 사용자 IP (IPv6 대응) |
| `viewed_at` | DATETIME | 조회 일시 |

---

## 🔧 구현 명세

### 1. 엔티티 생성

```java
@Entity
@Table(name = "board_view")
@Builder
@AllArgsConstructor
@NoArgsConstructor
@Getter
public class BoardView {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "board_id", nullable = false)
    private Board board;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "account_id")
    private Account account;  // nullable

    @Column(name = "ip_address", length = 45)
    private String ipAddress;  // nullable

    @CreationTimestamp
    @Column(name = "viewed_at", nullable = false)
    private LocalDateTime viewedAt;
}
```

### 2. Repository

```java
public interface BoardViewRepository extends JpaRepository<BoardView, Long> {
    
    // 로그인 사용자: 오늘 조회 기록 확인
    @Query("SELECT COUNT(bv) > 0 FROM BoardView bv " +
           "WHERE bv.board.id = :boardId " +
           "AND bv.account.id = :accountId " +
           "AND DATE(bv.viewedAt) = CURRENT_DATE")
    boolean existsTodayViewByAccount(
        @Param("boardId") Long boardId, 
        @Param("accountId") Long accountId
    );
    
    // 비로그인 사용자: 오늘 IP 기준 조회 기록 확인
    @Query("SELECT COUNT(bv) > 0 FROM BoardView bv " +
           "WHERE bv.board.id = :boardId " +
           "AND bv.ipAddress = :ipAddress " +
           "AND DATE(bv.viewedAt) = CURRENT_DATE")
    boolean existsTodayViewByIp(
        @Param("boardId") Long boardId, 
        @Param("ipAddress") String ipAddress
    );
    
    // 오래된 기록 삭제용
    void deleteByViewedAtBefore(LocalDateTime threshold);
}
```

### 3. Controller 수정 (IP 추출)

```java
@GetMapping("/{id}")
public ResponseEntity<BoardResponseDTO> getBoardById(
        @PathVariable Long id,
        @AuthenticationPrincipal CustomUserDetails userDetails,
        HttpServletRequest request
) {
    Long userId = userDetails != null ? userDetails.getUserId() : null;
    String clientIp = getClientIp(request);  // Controller에서 IP 추출
    return ResponseEntity.ok(boardService.getBoardById(id, userId, clientIp));
}

private String getClientIp(HttpServletRequest request) {
    String ip = request.getHeader("X-Forwarded-For");
    if (ip == null || ip.isEmpty() || "unknown".equalsIgnoreCase(ip)) {
        ip = request.getHeader("X-Real-IP");
    }
    if (ip == null || ip.isEmpty() || "unknown".equalsIgnoreCase(ip)) {
        ip = request.getRemoteAddr();
    }
    // X-Forwarded-For에 여러 IP가 있을 경우 첫 번째가 클라이언트 IP
    if (ip != null && ip.contains(",")) {
        ip = ip.split(",")[0].trim();
    }
    return ip;
}
```

### 4. Service 로직 수정

```java
// 인터페이스
BoardResponseDTO getBoardById(Long id, Long userId, String clientIp);

// 구현체
@Override
public BoardResponseDTO getBoardById(Long id, Long userId, String clientIp) {
    Board board = boardRepository.findById(id)
            .orElseThrow(() -> new BoardException(BoardErrorCode.BOARD_NOT_FOUND));
    
    // 삭제된 글 조회 불가 처리
    if (!board.isActive()) {
        throw new BoardException(BoardErrorCode.BOARD_NOT_FOUND);
    }
    if (board.isSecret() && (userId == null || !userId.equals(board.getAccount().getId()))) {
        throw new BoardException(BoardErrorCode.CANNOT_READ_BOARD);
    }
    
    // 조회수 증가 로직 (중복 방지)
    incrementHitsIfAllowed(board, userId, clientIp);
    
    BoardResponseDTO boardResponseDTO = BoardResponseDTO.from(board, userId);
    if (userId != null) {
        boardResponseDTO.setLike(
                board.getBoardRecommends().stream().anyMatch(br -> br.getAccount().getId().equals(userId)));
        boardResponseDTO.setOwner(userId.equals(board.getAccount().getId()));
    }
    return boardResponseDTO;
}

private void incrementHitsIfAllowed(Board board, Long userId, String clientIp) {
    // 1. 작성자 본인이면 증가 안함
    if (userId != null && userId.equals(board.getAccount().getId())) {
        return;
    }
    
    boolean alreadyViewed;
    
    if (userId != null) {
        // 2. 로그인 사용자: account_id 기준
        alreadyViewed = boardViewRepository.existsTodayViewByAccount(board.getId(), userId);
    } else {
        // 3. 비로그인 사용자: IP 기준
        alreadyViewed = boardViewRepository.existsTodayViewByIp(board.getId(), clientIp);
    }
    
    if (!alreadyViewed) {
        // 조회수 증가
        board.setHits(board.getHits() + 1);
        
        // 조회 기록 저장
        BoardView boardView = BoardView.builder()
                .board(board)
                .account(userId != null ? accountRepository.getReferenceById(userId) : null)
                .ipAddress(userId == null ? clientIp : null)
                .build();
        boardViewRepository.save(boardView);
    }
}
```

---

## 📁 생성/수정 파일 목록

### 신규 생성
| 파일 | 설명 | 상태 |
|------|------|------|
| `domain/board/BoardView.java` | 조회 기록 엔티티 | ✅ |
| `repository/board/BoardViewRepository.java` | 조회 기록 Repository | ✅ |
| `scheduler/BoardViewCleanupScheduler.java` | 30일 후 자동 삭제 스케줄러 | ✅ |

### 수정 완료
| 파일 | 수정 내용 | 상태 |
|------|----------|------|
| `service/board/BoardService.java` | 메서드 시그니처에 `String clientIp` 추가 | ✅ |
| `service/board/BoardServiceImpl.java` | 조회수 증가 로직 변경, `boardViewRepository` 주입 | ✅ |
| `controller/board/BoardController.java` | IP 추출 후 Service에 전달 | ✅ |

---

## 🔄 데이터 정리 (30일 후 자동 삭제)

```java
@Component
@RequiredArgsConstructor
public class BoardViewCleanupScheduler {
    
    private final BoardViewRepository boardViewRepository;
    
    // 매일 새벽 3시에 30일 이상 된 기록 삭제
    @Scheduled(cron = "0 0 3 * * ?")
    @Transactional
    public void cleanupOldViewRecords() {
        LocalDateTime threshold = LocalDateTime.now().minusDays(30);
        boardViewRepository.deleteByViewedAtBefore(threshold);
    }
}
```

> ⚠️ `@EnableScheduling`이 Application 클래스에 있어야 함

---

## ⚠️ 주의사항

1. **성능**: 조회마다 DB 체크 → 인덱스 필수
2. **IPv6**: `ip_address` 컬럼 45자 (IPv6 최대 길이)
3. **프록시**: `X-Forwarded-For` 헤더 우선 사용
4. **테스트**: 로그인/비로그인, 작성자 본인 케이스 모두 테스트 필요
5. **기존 데이터**: 기존 조회수(`hits`)는 그대로 유지됨
6. **스케줄링**: `@EnableScheduling`이 Application 클래스에 있어야 함

---

## 🧪 테스트 방법

1. **게시글 상세 페이지 접속** → 조회수 +1
2. **새로고침** → 조회수 변화 없음 (같은 날 중복 조회)
3. **작성자 본인 접속** → 조회수 변화 없음
4. **다른 브라우저/시크릿 모드로 접속** → 조회수 +1 (다른 IP/사용자)

---

## 📝 관련 문서

- [01_프론트엔드_연동_요청사항.md](./01_프론트엔드_연동_요청사항.md)
- [COMMUNITY_INTEGRATION_REPORT.md](./COMMUNITY_INTEGRATION_REPORT.md)
